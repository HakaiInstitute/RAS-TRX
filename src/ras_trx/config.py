...
class TrxCoordType(str, enum.Enum):
    ...
    def is_utm(self) -> bool:
        return self.value.startswith("UTM")
    ...
    def to_csrspy(self) -> CoordType:
        # Mapping directly to csrspy.enums.CoordType
        # Assuming csrspy.enums.CoordType has corresponding members
        return CoordType[self.name]
...
class ReferenceConfig(BaseModel):
    ...
    def crs(self) -> CRS:
        ...
        # For DEMs, we always expect a Z component, so if no vertical CRS is specified,
        # we assume the Z is part of the 2D CRS or is ellipsoidal height.
        # csrspy expects 3D coordinates, so we ensure the CRS is 3D if it's geographic.
