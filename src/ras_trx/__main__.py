...
from ras_trx.worker import TransformWorker
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.help_msg_box.setText(
            ...
            "<pre>C:\\\\path\\to\\files\\*.tif</pre>"
            "<p>Use '{}' in the output file to generate a name from the input name, <i>e.g.</i></p>"
            "<pre>C:\\\\path\\to\\files\\{}_transformed.tif</pre>"
        )
        self.help_msg_box.setWindowTitle("Help")

        # Setup window
        self.setWindowIcon(QIcon(resource_path("resources/ras-trx.ico")))
        self.setWindowTitle(f"RAS-TRX v{__version__}")
...
def main():
    ...
    if os.getenv("DEBUG"):
        window.lineEdit_input_file.setText(
            "24_4012_07_PlaceGlacier_DEM_1m_GF_WGS84_UTM10_Ellips.tif"
        )
        window.comboBox_input_reference.setCurrentText("WGS84")
        window.dateEdit_input_epoch.setDate(date(2024, 8, 2))
        window.comboBox_input_coordinates.setCurrentText("UTM")
        window.spinBox_input_utm_zone.setValue(10)
        window.comboBox_input_vertical_reference.setCurrentText("WGS84")

        window.lineEdit_output_file.setText(
            "24_4012_07_PlaceGlacier_DEM_1m_GF_WGS84_UTM10_Ellips_transformed.tif"
        )
        window.comboBox_output_reference.setCurrentText("NAD83(CSRS)")
        ...
        window.comboBox_output_coordinates.setCurrentText("UTM")
        window.spinBox_output_utm_zone.setValue(10)
